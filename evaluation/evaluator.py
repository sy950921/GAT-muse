...
from evaluation import get_wordsim_scores, get_crosslingual_wordsim_scores, get_wordanalogy_scores
from evaluation import get_word_translation_accuracy
from evaluation import load_europarl_data, get_sent_translation_accuracy
...
class Evaluator(object):
    ...
    def __init__(self, trainer):
        ...
        self.src_adj = trainer.src_adj
        self.tgt_adj = trainer.tgt_adj
    ...
    def monolingual_wordsim(self, to_log):
        ...
        src_ws_scores = get_wordsim_scores(
            ...
            self.src_mapping(self.src_emb.weight, self.src_adj).data.cpu().numpy()
        )
        tgt_ws_scores = get_wordsim_scores(
            self.tgt_dico.lang, self.tgt_dico.word2id,
            self.tgt_mapping(self.tgt_emb.weight, self.tgt_adj).data.cpu().numpy()
    ...
    def crosslingual_wordsim(self, to_log):
        ...
        src_emb = self.src_mapping(self.src_emb.weight, self.src_adj).data.cpu().numpy()
        tgt_emb = self.tgt_mapping(self.tgt_emb.weight, self.tgt_adj).data.cpu().numpy()
    ...
    def eval_dis(self, to_log):
        ...
        #     preds = self.discriminator(self.src_mapping(emb))
        ...
        with torch.set_grad_enabled(not volatile):
            src_emb = Variable(self.src_emb.weight.data).detach()
            tgt_emb = Variable(self.tgt_emb.weight.data).detach()
            src_preds = self.discriminator(self.src_mapping(src_emb, self.src_adj))
            tgt_preds = self.discriminator(self.tgt_mapping(tgt_emb, self.tgt_adj))

        src_list = list(src_preds)
        new_srclist = [x.cpu().detach().numpy() for x in src_list]
        src_pred = np.mean(new_srclist)
        tgt_list = list(tgt_preds)
        new_tgtlist = [x.cpu().detach().numpy() for x in tgt_list]
        tgt_pred = np.mean(new_tgtlist)
        logger.info("Discriminator source / target predictions: %.5f / %.5f"
                    % (src_pred, tgt_pred))

        src_accu = np.mean([x.cpu().detach().numpy() >= 0.5 for x in src_preds])
        tgt_accu = np.mean([x.cpu().detach().numpy() < 0.5 for x in tgt_preds])
